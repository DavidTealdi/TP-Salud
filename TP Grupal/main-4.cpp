...
void imprimirTurno(Nodo *&agenda, int codigoTurno){
    for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
        if(i->dato.codigo==codigoTurno){
            cout<<"Codigo del Turno: "<<i->dato.codigo<<endl;
            cout<<"DNI del Paciente: "<<i->dato.dni<<endl;
            cout<<"Codigo de la Especialidad Medica: "<<i->dato.codigoEspecialidad<<endl;
            cout<<"Fecha: "<<i->dato.fecha.dia<<"/"<<i->dato.fecha.mes<<"/"<<i->dato.fecha.anio<<endl;
            cout<<"Hora: "<<i->dato.horario.hora<<":"<<i->dato.horario.minuto<<" hs"<<endl;
            if(i->dato.estado==1){
...
void imprimirTurnosdePacientes(Nodo *&agenda, int dniPaciente){
    for(Nodo *i=agenda; i!=NULL; i=i->siguiente){                    //itera por la lista
        if(i->dato.estado==1 and i->dato.dni==dniPaciente){             //si el turno esta activo y pertenece al paciente lo imprime
            cout<<endl;
            imprimirTurno(agenda, i->dato.codigo);
        }
    }
};
...
/* busquedaEspecialidad
  ...
  PROPOSITO:  iterar sobre un arreglo de tipo Especialidad, buscando si existe la especialidad
              con el codigo indicado en los parametros
  PARAMETROS: se necesita del arreglo de tipo especialidad, el codigo de la especialidad que se busca y la
              dimension lógica del arreglo.
  RETORNO:    se retornará el lugar que ocupa en el arreglo el dni indicado o un -1 si no existe
              un paciente con ese dni.
...
int busquedaEspecialidad(Especialidad especialidades[], int & codEspMed, int & dl){
    for(int i=0; i<dl; i++){                        //itera por el arreglo de especialidades
        if(especialidades[i].codigo==codEspMed){    //si el dni del arreglo es igual al que se pasa por
            return i;                               //parametro retorna el lugar que ocupa en el arreglo
        }
    }
    return -1;      //sino existe en el arreglo retorna un -1
...
/* busquedaTurno
  ...
  PROPOSITO:  iterar sobre una lista de tipo Turno, buscando si existe el turno indicado
              con el codigo pasado por los parametros
  PARAMETROS: se necesita una lista de tipo Turno, y el codigo que se busca.
  RETORNO:    se retornará un true si existe el turno o si no existe un false.

*/

bool busquedaTurnos(Nodo *&agenda, int & codTurno){
    for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
        if(i->dato.codigo==codTurno){
            return true;
        }
    }
    return false;
...
Hora solicitarHora(){
    ...
    cout<<"\nDebes tener en cuenta que el horario de atencion de la clinica es de 8hs a 19hs,\ny que nuestro turnos son de 30 minutos. "<<endl;
    cout<<"Ingrese el horario:"<<endl;
    cout<<"Hora: ";
    cin>>hora;
    while(hora<8 or hora>18){
...
int cantTurnosDelPaciente(int & dniPaciente, Nodo *&agenda){
    int contador=0;
    for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
        if(i->dato.dni==dniPaciente and i->dato.estado==1){
            contador++;
        }
    }
    return contador;
...
bool turnoDeEspecialidadActivo(int & dniBuscado, Nodo *&agenda, int & codEspecialidad){
    for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
        if(i->dato.dni==dniBuscado and i->dato.codigoEspecialidad==codEspecialidad and i->dato.estado==1){
            return true;
        }
    }
    return false;
...
bool tieneTurnosEspecialidad(int & codigoBuscado, Nodo *&agenda){
    for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
        if(i->dato.codigoEspecialidad==codigoBuscado and i->dato.estado==1){
            return true;
        }
    }
    return false;
...
void Alta_de_Especialidades_Medicas(Especialidad especialidades[], int & dl, int & codEspMed){
    Especialidad nuevaEspecialidad; char eleccion; bool repetir;

    //ASIGNA EL CODIGO UNICO DE LA ESPECIALIDAD
    nuevaEspecialidad.codigo=codEspMed;
    ...
    cout<<"La especialidad quedo asi: "<<endl;
    imprimirEspecialidad(especialidades, codEspMed);
    codEspMed++;
...
void Modificacion_de_Especialidades_Medicas(Especialidad especialidades[], int &dl){
    int codigoEspecialidad, lugarArreglo; Especialidad modificaciones;
    char eleccion; bool repetir;

    cout<<"Ingresar el codigo de la Especialidad a Modificar: "<<endl;
    cin>>codigoEspecialidad;
    lugarArreglo=busquedaEspecialidad(especialidades,codigoEspecialidad, dl);

    while(lugarArreglo==-1){
        cout<<"La especialidad indicada no existe";
        cin>>codigoEspecialidad;
        lugarArreglo=busquedaEspecialidad(especialidades,codigoEspecialidad, dl);
    }
    cout<<"Se va a modificar "<<especialidades[lugarArreglo].nombre<<endl;
    ...
    do{
        ...
        case 'N':
            ...
            modificaciones.nombre=especialidades[lugarArreglo].nombre;
            break;
    ...
    do{
        ...
        case 'N':
            modificaciones.descripcion=especialidades[lugarArreglo].descripcion;
            break;
    ...
    especialidades[lugarArreglo]=modificaciones;
    imprimirEspecialidad(especialidades,lugarArreglo);
...
void Baja_de_Especialidades_Medicas(Especialidad especialidades[], int & dlEspecilidades, Nodo *&agenda){
    int codigoEliminar, lugarArreglo; char eleccion; bool repetir;

    //SOLICITA AL USUARIO QUE
    cout<<"Ingresar el codigo de la Especialidad Medica: ";
    cin>>codigoEliminar;
    lugarArreglo=busquedaEspecialidad(especialidades,codigoEliminar, dlEspecilidades);

    //revisa que exista tal codigo, sino es asi vuelve al menu principal
    if(lugarArreglo==-1){
        cout<<"La Especialidad Medica indicada no existe dentro del sistema."<<endl;
        system("pause");
        system("cls");
        return;
    }
    ...
    do{
        ...
        case 'N':
            cout<<"No se ha eliminado a la Especialidad Medica "<<codigoEliminar<<endl;
            system("pause");
            system("cls");
            break;
        ...
        case 'S':
            ...
            if(tieneTurnosEspecialidad(codigoEliminar, agenda)==true){
...
void Listado_de_Especialidades_Medicas_por_busqueda(Especialidad especialidades[], int & dl){
    ...
    int codigoBusqueda, lugarArreglo;

    cout<<"Ingresar el codigo de la Especialidad Medica: ";
    cin>>codigoBusqueda;
    lugarArreglo=busquedaEspecialidad(especialidades,codigoBusqueda,dl);
...
void Modificacion_de_Pacientes(Paciente fichaPaciente[], int & dl){
    int dniModificar, espacioFicha; char eleccion; Paciente modificaciones; bool repetir;
...
void Baja_de_Pacientes(Paciente fichasPacientes[], int & dlFichas, Nodo *&agenda){
    int dniEliminar, espacioFicha; char eleccion; bool repetir;
    ...
    do{
        ...
        case 'N':
            cout<<"No se ha eliminado al paciente del DNI "<<dniEliminar<<endl;
            system("pause");
            system("cls");
            break;
        ...
        case 'S':
            ...
            if(cantTurnosDelPaciente(dniEliminar,agenda)>0){
...
void Alta_de_Turnos(Nodo *&agenda, Paciente fichas[], Especialidad especialidades[],int & dlPacientes, int & dlEspecialidades, int & dlTurno, Tiempo fechaActual, int & codigoTurnos){

    int dniPaciente, codEspecialidad;
    Turno nuevo;

    nuevo.codigo=codigoTurnos;
    ...
    cin>>codEspecialidad;
    if(busquedaEspecialidad(especialidades, codEspecialidad, dlEspecialidades)==-1){
    ...
    if(turnoDeEspecialidadActivo(dniPaciente, agenda, codEspecialidad)==true){
    ...
    while((aux1!= NULL) && (aux1->dato.codigo<nuevo.codigo)){
        aux2=aux1;
        aux1=aux1->siguiente;
    }
    if(agenda==aux1){
    ...
    imprimirTurno(agenda,codigoTurnos);
    codigoTurnos++;
    dlTurno++;
...
void Modificacion_de_Turnos(Paciente fichas[], int & dlPacientes, Nodo *&agenda, int & dlAgenda, Tiempo hoy){
    int dniPaciente, codigoModificar; char eleccion; bool repetir;
    ...
    if(cantTurnosDelPaciente(dniPaciente,agenda)==0){
    ...
    imprimirTurnosdePacientes(agenda, dniPaciente);
    ...
    for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
        if(i->dato.codigo==codigoModificar){
            ...
            do{
                ...
                case 'S':
                    modificaciones.estado=1;
                    i->dato=modificaciones;
...
void Baja_de_Turnos(Paciente fichas[], int & dlPacientes, Nodo *&agenda, Tiempo hoy){
    int dniPaciente, cantTurnos, codigoEliminar; bool validacion, repetir; char eleccion; Tiempo fechaLimite;

    fechaLimite=hoy;
    fechaLimite.dia=fechaLimite.dia+3;//para calcular las 48 hs antes del turno
    ...
    cantTurnos=cantTurnosDelPaciente(dniPaciente,agenda);

    if(cantTurnos==0){
        cout<<"El paciente con el DNI "<<dniPaciente<<" no tiene turnos activos."<<endl;
        system("pause");
        system("cls");
        return;
    }
    ...
    if(cantTurnos==1){
        ...
        for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
            if(i->dato.dni==dniPaciente and i->dato.estado==1){
                validacion=validarFecha(i->dato.fecha.dia, i->dato.fecha.mes, i->dato.fecha.anio, fechaLimite);
                if(validacion==true){
    ...
    if(cantTurnos>1){
        ...
        while(busquedaTurnos(agenda, codigoEliminar)==false){
            cout<<"El codigo ingresado no pertence a ningun turno otorgado, vuelva a intentarlo: ";
            cin>>codigoEliminar;
        }

        for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
            if(i->dato.codigo==codigoEliminar and i->dato.dni==dniPaciente){
                validacion=validarFecha(i->dato.fecha.dia, i->dato.fecha.mes, i->dato.fecha.anio, fechaLimite);
                if(validacion==true){
                   do{
...
void Listado_de_Turnos_completo(Nodo *&agenda){
...
void Listado_de_Turnos_por_busqueda(Nodo *&agenda, Paciente fichaPaciente[], int & dlPaciente, Especialidad especialidades[], int & dlEspecialidad){
    ...
    do{
        ...
        case 0: //caso de que busque filtrar por medio de una fecha
            ...
            for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
                if(fechaBuscar.dia==i->dato.fecha.dia and fechaBuscar.mes==i->dato.fecha.mes and fechaBuscar.anio==i->dato.fecha.anio){
                    imprimirTurno(agenda, i->dato.codigo);
                    cout<<endl;
                }
            }
        ...
        case 1: //caso de que busque filtrar por medio de un dni
            ...
            //por medio de una funcion verifica que el paciente este ingresado en el sistema
            //si el paciente no esta en el sistema, se le indica a el usuario que ingrese otro dni.
            if(busquedaPaciente(fichaPaciente, dni, dlPaciente)==-1){
                cout<<"El dni ingresado no esta ingresado en el sistema."<<endl;
                system("pause");
                system("cls");
                break;
            ...
            imprimirTurnosdePacientes(agenda,dni);
        ...
        case 2: //caso de que busque filtrar por medio de la especialidad medica
            ...
            //revisa que exista tal codigo, sino es asi vuelve al menu principal
            if(busquedaEspecialidad(especialidades, codigoEspecialidad, dlEspecialidad)){
                cout<<"La Especialidad Medica indicada no existe dentro del sistema.";
                system("pause");
                system("cls");
                break;
            ...
            for(Nodo *i=agenda; i!=NULL; i=i->siguiente){
                if(i->dato.codigoEspecialidad==codigoEspecialidad){
                    cout<<endl;
                    imprimirTurno(agenda,i->dato.codigo);
                }
            }
...
void submenuAdministrarEspecialidadesMedicas(Especialidad especialidades[], int & dl, Nodo *&agenda, int & codEspMed){
    ...
    do{
        ...
        switch(eleccion){
            ...
            case '1':
                Alta_de_Especialidades_Medicas(especialidades, dl, codEspMed);
            break;
            ...
            case '3':
                Baja_de_Especialidades_Medicas(especialidades, dl, agenda);
            break;
...
void submenuAdministrarPacientes(Paciente fichaPaciente[], int & dlFichas, Nodo *&agenda){
    ...
    do{ //Se abre un bucle donde el primero se
        ...
        switch(eleccion){
            ...
            case '3':
                Baja_de_Pacientes(fichaPaciente, dlFichas, agenda);
                break;
...
void submenuAdministrarTurnos(Nodo *&agenda, Paciente fichas[], Especialidad especialidades[], int & dlPacientes, int & dlEspecialidades, int & dlTurno, Tiempo diaDeHoy, int & codigosTurnos){
    ...
    do{
        ...
        switch(eleccion){
            ...
            case '1':
                Alta_de_Turnos(agenda, fichas, especialidades, dlPacientes, dlEspecialidades, dlTurno, diaDeHoy, codigosTurnos);
            break;
            ...
            case '3':
                Baja_de_Turnos(fichas, dlPacientes, agenda, diaDeHoy);
            break;
            ...
            case '4':
                ...
                do{
                    ...
                    switch(subEleccion){
                        ...
                        case 'a':
                            Listado_de_Turnos_completo(agenda);
                            return;

                        case 'b':
                            Listado_de_Turnos_por_busqueda(agenda, fichas, dlPacientes, especialidades, dlEspecialidades);
                            return;
...
int main(){
    ...
    Paciente fichas[MAX];
    Nodo *agenda=NULL;
    int dlFichas, dlEspecialidades, dlAgenda, codigosEspecialidadesMedicas=0, codigosTurnos=0;
    ...
    do{
        ...
        switch(opcion) {
            ...
            case '1':
            system("cls"); // Limpia la consola
            submenuAdministrarEspecialidadesMedicas(especialidades, dlEspecialidades, agenda, codigosEspecialidadesMedicas);   // Luego llama a la funcion
            break;

            // Submenu 2
            case '2':
            system("cls"); // Limpia la consola
            submenuAdministrarPacientes(fichas, dlFichas, agenda);  // Luego llama a la funcion
            ...
            case '3':
                ...
                submenuAdministrarTurnos(agenda, fichas, especialidades, dlFichas, dlEspecialidades, dlAgenda, diaDeHoy, codigosTurnos);  // Luego llama a la funcion
